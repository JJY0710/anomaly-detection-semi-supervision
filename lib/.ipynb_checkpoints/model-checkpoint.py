...
class GANomaly2D(nn.Module):
    def __init__(self, r = 1, device = 'cpu'):
        ...
        self.G_E = Encoder(in_channels  = 3, nef = 120 // r)
#         self.BLO = 
        self.G_D = Decoder(out_channels = 3, ndf = 120 // r)
        self.E = Encoder(in_channels = 3, nef = 120 // r)
        self.f = Discriminator(in_channels = 3, out_channels = 1, nef = 120 // r, last = nn.Sigmoid)
        ...
        self.optim_G = Adam(itertools.chain(self.G_E.parameters(), self.G_D.parameters(), self.E.parameters()), lr = 0.0002)
        self.optim_D = Adam(self.f.parameters(), lr = 0.0002)
        self.to(self.device)
